class WebSocketClient {
  ...
  connect() {
    try {
      ...
      if (!token) {
        console.warn($t('messages._____token_____websocket'));
        return;
      }
  ...
  onOpen(event) {
    console.log($t('messages.websocket_____'));
  ...
  onMessage(event) {
    try {
      ...
      console.log($t('messages.__websocket___'), data);
  ...
  onClose(event) {
    console.log($t('messages.websocket______'), event.code, event.reason);
    this.isConnected = false;
    this.stopHeartbeat();
  ...
  send(type, data = {}) {
    ...
    try {
      ...
      console.error($t('messages.__websocket_____'), error);
      return false;
    }
  }
  ...
  updateRedpacketButton(isActive) {
    ...
    redpacketButtons.forEach(button => {
      if (isActive) {
        ...
        button.textContent = $t('messages.grab');
