...
class TeamController {
      ...
      'default': [
        ...
        },
        {
          ...
          members: [
            {
              userId: 'user004',
              username: $t('messages.__004'),
              joinTime: Date.now() - 259200000,
              status: 2
            },
            {
              userId: 'user005',
              username: $t('messages.__005'),
  ...
  getTeamInfo = async (req, res) => {
    try {
      ...
      const teamStructure = this.teamStructure[userId] || [];
      const teamStats = this.teamStats[userId] || {
        totalMembers: 0,
        directMembers: 0
      };
      ...
      res.json({
        ...
        data: {
          ...
          directMembers: teamStats.directMembers,
          teamStructure
        },
        timestamp: Date.now()
      });
  ...
  generateInviteLink = async (req, res) => {
    try {
      ...
      const inviteLink = `https://domain.com/register?code=${inviteCode}`;
      const expiryTime = Date.now() + (120 * 60 * 60 * 1000); // 120小时后过期
      const remainingHours = 120;
  ...
  async processInvite(req, res) {
    try {
      ...
      if (!inviteCode || !newUserId || !username) {
        return res.status(400).json({
          success: false,
          message: $t('messages.______id________')
        });
      }
