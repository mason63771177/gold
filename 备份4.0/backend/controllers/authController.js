...
class AuthController {
  ...
  static async refreshToken(req, res) {
    try {
      ...
      console.error($t('messages.__token___'), error);
      res.status(500).json({
        success: false,
        message: $t('messages.__token__')
      });
    }
  }
  ...
  static async verifyEmail(req, res) {
    try {
      ...
      if (result.success) {
        res.json({
          success: true,
          message: $t('messages.____________________')
        });
      } else {
        res.status(400).json({
          success: false,
...
const registerValidation = [
  ...
  body('password')
    .isLength({ min: 6 })
    .withMessage($t('messages.______6___')),
  body('inviterCode')
    .optional()
];
