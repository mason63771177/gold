...
class TaskController {
  ...
  initializeDefaultTasks() {
    this.defaultNewbieTasks = [
      { id: 1, title: '直接推荐1人', desc: $t('messages.__1__________'), done: false, reward: 10 },
      { id: 2, title: $t('messages.______1_'), desc: $t('messages.__________1____'), done: false, reward: 10 },
      { id: 3, title: $t('messages.____________1_'), desc: '培训下级的推广技巧，帮助三级推广', done: false, reward: 10 }
    ];

    this.defaultQuizTasks = [
      { id: 'quiz1', title: '基础知识答题', desc: $t('messages.__20___________80_'), done: false, reward: 20, feeReduction: 0.02 }
    ];

    this.defaultGodTasks = [
      { id: 1, title: $t('messages.________2_'), desc: $t('messages.__2________2_'), done: false, reward: 50 },
      { id: 2, title: $t('messages.________3_'), desc: $t('messages.__3________3_'), done: false, reward: 250 },
      { id: 3, title: $t('messages.________4_'), desc: $t('messages.__4________4_'), done: false, reward: 1250 },
      { id: 4, title: $t('messages.________5_'), desc: $t('messages.__5________5_'), done: false, reward: 6250 },
      { id: 5, title: $t('messages.________6_'), desc: $t('messages.__6________6_'), done: false, reward: 31250 },
      { id: 6, title: $t('messages.________7_'), desc: $t('messages.__7________7_'), done: false, reward: 156250 }
    ];
  }
  ...
  getTaskList = async (req, res) => {
    try {
      ...
      const quizTasks = userTasks.quizTasks.map(task => ({
        taskId: task.id,
        question: task.title,
        options: [$t('messages.a__________'), $t('messages.b________'), $t('messages.c______'), $t('messages.d______')],
  ...
  answerQuiz = async (req, res) => {
    try {
      ...
      if (!questionId || !answer) {
        return res.status(400).json({
          code: 400,
          message: $t('messages.__id_______'),
          timestamp: Date.now()
        });
      }
  ...
  completeQuizTask = async (req, res) => {
    try {
      ...
      if (correctRate < 0.8) {
        return res.status(400).json({
          success: false,
          message: $t('messages._____80_______')
        });
      }
