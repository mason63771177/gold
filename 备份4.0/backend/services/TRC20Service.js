...
class TRC20Service {
  constructor() {
    ...
    this.addressPool = [];
    this.monitoringAddresses = new Map(); // 监听中的地址
  }
  ...
  async initialize() {
    ...
    try {
      ...
      console.error($t('messages.trc20service_______'), error);
      throw error;
    }
  }
  ...
  async generateAddressPool(count = 50) {
    try {
      ...
      for (let i = 0; i < count; i++) {
        const addressData = this.generateTRC20Address();
  ...
  generateTRC20Address() {
    ...
    const hash = crypto.createHash('sha256')
      .update(Buffer.concat([Buffer.from(timestamp.toString()), randomBytes]))
      .digest();
    
    // TRC20地址：T + 33位Base58字符
  ...
  async assignWalletAddress(userId, type = 'activation') {
    try {
      // 获取可用地址
      let walletAddress = await WalletAddress.getAvailableAddress();
      
      // 如果没有可用地址，生成新的
      if (!walletAddress) {
        console.log('地址池不足，生成新地址...');
        const addressData = this.generateTRC20Address();
        walletAddress = await WalletAddress.create(addressData);
      }
      
      // 分配给用户
      await walletAddress.assignToUser(userId);
      
      // 开始监听这个地址
      this.startAddressMonitoring(walletAddress.address);
