...
class EmailService {
  ...
  async initializeTransporter() {
    try {
      ...
      console.log($t('messages._______ethereal_______'));
      await this.createEtherealTestAccount();
    }
  }
  ...
  async createEtherealTestAccount() {
    try {
      ...
      console.log($t('messages.__ethereal________'));
      console.log('📧 测试邮箱:', testAccount.user);
      console.log('🔗 邮箱管理: https://ethereal.email/login');
    } catch (error) {
      console.error($t('messages.____ethereal_______'), error);
      this.isConfigured = false;
    }
  }
  ...
  async sendVerificationEmail(email, verificationToken, userName = '') {
    if (!this.isConfigured) {
      console.warn($t('messages.________________'));
