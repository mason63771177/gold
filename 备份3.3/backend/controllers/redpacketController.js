...
class RedpacketController {
  ...
  initializeTestData() {
    ...
    this.userRecords.set(defaultUserId, [
      {
        id: 'rp001',
        amount: 15.50,
  ...
  grabRedpacket = async (req, res) => {
    try {
      ...
      const userRecords = this.userRecords.get(userId) || [];
      const today = new Date().toDateString();
      ...
      // 生成随机红包金额
      const amount = parseFloat((Math.random() * (this.maxAmount - this.minAmount) + this.minAmount).toFixed(2));
      ...
      // 更新今日统计
      const dailyStats = this.dailyRecords.get(today) || {
        totalGrabbed: 0,
        totalAmount: 0,
        participants: 0
      };
      dailyStats.totalGrabbed += 1;
      dailyStats.totalAmount += amount;
      this.dailyRecords.set(today, dailyStats);
      
      // 模拟更新钱包余额
      const currentBalance = 160.50; // 这里应该从钱包系统获取
      ...
      res.json({
        ...
        data: {
          ...
          newBalance,
          rank,
          totalGrabbed: dailyStats.totalGrabbed
        },
        timestamp: Date.now()
      });
    } catch (error) {
